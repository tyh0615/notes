...
getFood(false); // undefined1234567891011
```
...
getFood(false); // 'Meow Mix'1234567891011
```
...
let x = 'hi'; // ReferenceError: x is not defined123
```
...
console.log(food); // Reference Error12345
```
...
```javascript
var squares = arr.map(function (x) { return x * x }); // Function Expression1
const arr = [1, 2, 3, 4, 5];
const squares = arr.map(x => x * x); // Arrow Function for terser implementation12
```
...
console.log(string.indexOf(substring) > -1);1234
```
...
console.log(string.includes(substring)); // true1234
```
...
```javascript
var text = "This string contains \"double quotes\" which are escaped.";1
let text = `This string contains "double quotes" which don't need to be escaped anymore.`;1
```
...
console.log('My cat is named ' + name + ' and is ' + age + ' years old.');1234
```
...
```javascript
let today = new Date();
let text = `The time and date is ${today.toLocaleString()}`;12
```
...
var father = luke.father; // 'anakin'123
let luke = { occupation: 'jedi', father: 'anakin' };
let {occupation, father} = luke;

console.log(occupation); // 'jedi'
console.log(father); // 'anakin'12345
```
...
module.exports = ['foo', 'bar'];
module.exports = function bar () {};1234
```
...
export { sumTwo, sumThree };123456789
```
...
export default api;1234567891011121314
```
...
```javascript
import { sumTwo, sumThree } from 'math/addition';1
```
...
} from 'math/addition';1234
```

另外，我们能 **引入所有的东西（原文：import all the things）** （也称为命名空间引入）

```javascript
import * as util from 'math/addition';1
```
...
const { sumTwo, sumThree } = additionUtil;12
```
...
// Same as: import { default as api } from 'math/addition';12
```
...
```javascript
// foos.js
export { foo as default, foo1, foo2 };12
```

那我们可以如此导入这个模块的值：

```javaqscript
import foo, { foo1, foo2 } from 'foos';1
```
...
const { Component, PropTypes } = React;12
```

更简化版本：

```javascript
import React, { Component, PropTypes } from 'react';1
```
...
function initializeCanvas(
    ...
    // Use variables height, width, lineStroke here12345
```
...
Symbol.for('foo') === Symbol.for('foo') // true123
```
...
> TypeError: Property 'hasOwnProperty' is not a function12
```
...
value = map.get(el); // undefined1234567891011121314
```
...
new Promise((resolve, reject) =>
    reject(new Error('Failed to fulfill Promise')))
        .catch(reason => console.log(reason));123
```
...
> console.log(generator.next()); // { value: 4, done: false }123456789101112
```
...
main();12345678910111213141516
```

强烈推荐使用 `async await` 来替代Generators + Promises的写法。 
